...
fn validate_windows_shell_execute_result(result: isize) -> Result<()> {
    if result > 32 {
        Ok(())
    } else {
        Err(Error::WindowsInstallerLaunchFailed(result))
    }
}
...
impl Update {
    ...
    fn install_inner(&self, bytes: &[u8]) -> Result<()> {
        ...
        validate_windows_shell_execute_result(launch_result)?;
...
mod tests {
    ...
    #[test]
    fn windows_shell_execute_requires_a_result_above_32() {
        for result in [0, 2, 5, 31, 32] {
            assert!(super::validate_windows_shell_execute_result(result).is_err());
        }
        assert!(super::validate_windows_shell_execute_result(33).is_ok());
    }
